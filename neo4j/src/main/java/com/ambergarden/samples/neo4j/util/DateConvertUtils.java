...
public class DateConvertUtils {
	 ...
	 * 用来在线程内缓存日期格式器
	 */
	private static ThreadLocal<Map<Key, DateConvertUtils>> formatMapThreadLocal = new ThreadLocal<Map<Key, DateConvertUtils>>();
	 ...
	 * @created 2011-6-14 下午01:42:18
	 */
	private DateConvertUtils(SimpleDateFormat dateFormat, Locale locale) {
	 ...
	 */
	public static DateConvertUtils getInstance(String pattern, TimeZone timeZone,
			Locale locale) {
		Map<Key, DateConvertUtils> formatMap = formatMapThreadLocal.get();
		if (formatMap == null) {
			formatMap = new HashMap<Key, DateConvertUtils>();
			formatMapThreadLocal.set(formatMap);
		}

		Key key = new Key(pattern, locale);

		DateConvertUtils format = formatMap.get(key);
		if (format == null) {
			format = new DateConvertUtils(new SimpleDateFormat(pattern, locale),
					locale);
	 ...
	 * @created 2011-6-22 下午04:02:46
	 */
	public static DateConvertUtils getInstance(String pattern) {
